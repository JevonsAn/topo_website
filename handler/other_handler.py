...
class OtherHandler(BaseHandler, ABC):
    def prepare(self):
    ...
    async def get(self):
        ...
        if action == "pop" and typE == "neighbor":
            ...
            def getTaskInfofromTablename(tablename):
                ...
                timestamp = sp[3]
                # print(sp, timestamp)
                return "N0700_" + timestamp + "_0000", time.strftime("%Y-%m-%d %H:%M:%S",
                                                                     time.localtime(float(timestamp[:-3])))
            ...
            if typE == "edge":
                ...
                out_ip = args["out_ip"]
                if in_ip and out_ip:
                    isSuccess, tables = linkChangeConnManage.execute_and_fetch(
                        "select table_name from information_schema.tables where table_schema='edges' and table_type='base table' and table_name like '%N0700%';")
                    if not isSuccess:
                        raise tables
                    # print("=======================")
                    # print(tables)
                    # print("=======================")
                    edge_table_names = [t["table_name"] for t in tables if
                                        t["table_name"].endswith("edge_table") and not t["table_name"].startswith(
                                            "_parse")]
                    for tablename in edge_table_names:
                        isSuccess, have_num = linkChangeConnManage.execute_and_fetch(
                            "select count(*) as c from %s where (in_ip, out_ip) in (('%s', '%s'), ('%s', '%s'));" % \
                    ...
                    isSuccess, r = linkChangeConnManage.execute_and_fetch(
                        "select first_seen, last_seen from edge_table where (in_ip, out_ip) = ('%s', '%s') limit 1" % (
                            in_ip, out_ip))
                    if not isSuccess:
                        raise r
                    # r=r[0]
                ...
                if ip:
                    ...
                    if not isSuccess:
                        raise tables
                    node_table_names = [t["table_name"] for t in tables if
                                        t["table_name"].endswith("node_table") and not t["table_name"].startswith(
                                            "_parse")]
                    # print(node_table_names)
                    for tablename in node_table_names:
                        isSuccess, have_num = linkChangeConnManage.execute_and_fetch(
                            "select count(*) as c from %s where ip='%s'" % (tablename, ip))
                        if not isSuccess:
                            raise have_num
                        have_num = have_num[0]["c"]
                        if have_num:
                    ...
                    isSuccess, r = linkChangeConnManage.execute_and_fetch(
                        "select first_seen, last_seen from node_table where ip='%s' limit 1" % (ip,))
                    if not isSuccess:
                        raise r
                    # r = r[0]
                    first_seen = str(r[0]["first_seen"])
